    ...
    pub fn prepare(
        ...
        for ((transition_id, caller_dynamic_record_id), index) in self.transition_indices.iter() {
            if let Some((translation_tasks, record_translation_data)) = self.translation_tasks.get(transition_id) {
                ...
                let mut found = translation_tasks.iter().zip(record_translation_data.iter()).filter(|(_, data)| data.register_index == *index).collect_vec();
                ...
                ensure!(register_index == index, "Expected register index in translation data to be the same as the index in translation tasks for transition ID {} and register index {}", transition_id, index);
