        ...
        // If the operator is a function, retrieve the function and compute the output.
            ...
            if let CallStack::AuthorizeRequests(requests, current_index, authorization) = &mut call_stack {
                ...
                let request = requests.get(*current_index).ok_or_else(||
                    anyhow!("CallDynamic::evaluate attempted to retrieve request at index {current_index}, but the AuthorizeRequests call stack only contains {} request(s)",
                    requests.len())
                )?;

                // Sanity checks
                if *request.signer() != registers.signer()? {
                    return Err(anyhow!("CallDynamic::evaluate retrieved a request with a different signer than the current signer").into());
                }
                ...
                if request.inputs() != inputs {
                    return Err(anyhow!("CallDynamic::evaluate retrieved a request with a different inputs than the ones passed to the call instruction").into());
                }
