...
// Structure that keeps track of the global record-existence check of an
// execution, i. e. that each `DynamicRecord` and `ExternalRecord` input to the
// root transition corresponds to a static `Record` that exists on the ledger at
// the end of the execution.
struct GlobalCheck<N: Network> {
    // Each element of the vector corresponds to a family of registers that are
    // connected to a `DynamicRecord` or `ExternalRecord` input to the root
    // transition.
    families: Vec<IndexSet<(N::TransitionID, u64)>>,
}

impl<N: Network> GlobalCheck<N> {
    // Initialises the global check with an empty collection of register
    // families. Families are created only once: when processing the inputs to
    // the root transition.
    fn new() -> Self {
    ...
    // Adds a record to the family containing another record, if any, thus
    // connecting their existence status.
    ...
    // Marks the family containing a given record register as existing on the
    // ledger by removing it from the vector of families.
    fn mark_existing(&mut self, record: (N::TransitionID, u64)) {
        ...
        let family = self.families.iter().position(|family| family.contains(&record));

        if let Some(family_index) = family {
            self.families.remove(family_index);
        }
    }

    // Returns the families of registers that are not yet known to correspond to
    // static Records on the ledger.
    fn non_existent_families(&self) -> &[IndexSet<(N::TransitionID, u64)>] {
        &self.families
    }
}

// Auxiliary structure for the local record-existence check of a function, i. e.
// the check that, for each DynamicRecord cast from a locally minted static
// Record and passed to a non-closure call or output, the static Record is
// output.
struct FunctionLocalCheck {
    // Registers of static Records minted in this transition.
    locally_minted_static: HashSet<u64>,
    // For each DynamicRecord at r_j cast from a locally minted static Record at
    // r_i, this map contains the entry r_j -> r_i. This also takes into account
    // remaps of DynamicRecord caused by external-to-dynamic casts and
    // dynamic-to-static casts occurring in closures.
    locally_minted_dynamic: HashMap<u64, u64>,
    // Contains the registers of locally minted static Records which must be
    // output because they are cast to dynamic and passed to a non-closure call
    // or output.
...
impl FunctionLocalCheck {
    ...
    // Tracks the information about a static-to-dynamic cast as long as the
    // operand corresponds to a locally minted static Record.
    ...
    // Returns whether a given register corresponds to a locally minted static Record.
    fn is_locally_minted_static(&self, register: u64) -> bool {
        self.locally_minted_static.contains(&register)
    }

    // Returns the register of the static Record from which a given
    // DynamicRecord is cast, if any.
    fn get_static_source_of_dynamic(&self, register: u64) -> Option<u64> {
        self.locally_minted_dynamic.get(&register).cloned()
    }

    // Keeps track of the fact that a register has been passed to a function
    // call: if the register corresponds to a DynamicRecord cast from a locally
    // minted static Record, this keeps track of the fact that the static Record
    // must be output.
    ...
    // Adds a register to the set of registers that must be output because they
    // are cast to a DynamicRecord and passed to a non-closure call or output.
    fn add_must_be_output(&mut self, register: u64) {
        self.must_be_output.insert(register);
    }

    // Given a list of which registers are output by the function, checks that
    // all registers that must be output according to the local check are
    // actually so. If one or more registers break that condition, returns
    // Some(i), where r_i is one of them.
...
// Auxiliary structure for the local record-existence check of a closure, i. e.
// the check that each DynamicRecord cast from a static Record minted in the
// closure is not output.
...
fn process_transition<N: Network>(
    ...
    process: &Process<N>,
    // Global check keeping track of families of registers connected to
    // `DynamicRecord`s or `ExternalRecord`s input to the root transition.
    ...
    if let Some((caller_tid, caller_input_registers, _)) = &caller_info {
            ...
            izip!(caller_input_registers, input_registers, inputs)
        {
            if let Some(caller_input_register) = caller_input_register_opt {
                match callee_input {
                    ...
                    // TODO (Antonio) add External -> Static case
        ...
        // Note: even if record_families is empty, we need to process the transitions to enforce the local check
    }
            ...
            Instruction::Cast(cast) => {
                match cast.cast_type() {
                    ...
                    CastType::DynamicRecord => {
                        ...
                        // register if the old register exists in some family, this call only handles external-to-dynamic casts.
                        global_check.add_to_family(old_register, new_register);

                        // Case 5: Local-check update. This keeps track of the cast if the operand is a locally minted static Record.
                    ...
                    && !call.is_function_call(stack.as_ref())?
                {
                    ...
                    for input_register in caller_input_operands.iter() {
                        if let Operand::Register(register) = input_register {
                            // Case 6: Any DynamicRecord which is passed to a non-closure call and was cast
                            // from a locally minted static Record must be output.
    ...
    function.outputs().iter().for_each(|output| {
        if let Operand::Register(output_register) = output.operand() {
            if let Some(static_record) = local_check.get_static_source_of_dynamic(output_register.locator()) {
                // TODO (Antonio) add test case
                local_check.add_must_be_output(static_record);
            }
        }
    });

    // In a second pass, ensure all static records which should be output according to the local check actually are
    let actually_output = HashSet::from_iter(function.outputs().iter().filter_map(|output| {
        if let Operand::Register(output_register) = output.operand() { Some(output_register.locator()) } else { None }
    }));

    if let Some(not_output) = local_check.check_all_are_output(actually_output) {
        bail!(
...
// Auxiliary function for ensure_records_exist which processes a closure. The
// global check and the caller function's local check are updated taking into
// account the cast instructions in the closure as well as its input-output
// relations. Furthermore, this function ensures the closure does not output any
// DynamicRecords cast from locally minted static Records.
                    ...
                    if local_check.is_locally_minted_static(operand_register) {
                        ...
                        if let Some(caller_destination_register) = output_map.get(&destination_register) {
                            if caller_local_check.is_locally_minted_static(*caller_operand_register) {
                                // Case 3: Effectively performing a static-to-dynamic cast in the caller. We update the caller's local check accordingly.
                                caller_local_check
