...
use crate::{CallStack, InclusionVersion, Process, Trace, execution_cost, execution_cost_for_authorization, ToField};
use circuit::{Aleo, network::AleoV0};
use console::{
    account::{Address, PrivateKey, ViewKey},
    network::{MainnetV0, prelude::*},
    program::{DynamicRecord, Identifier, Literal, Plaintext, ProgramID, Record, Value, compute_function_id},
    types::{Field, U16, U64},
};
...
#[test]
fn test_translation_case_1() {

    // Declare the function name.
    let function_name_1 = Identifier::<CurrentNetwork>::from_str("consume_container").unwrap();
    let function_name_2 = Identifier::<CurrentNetwork>::from_str("get_liquid_liters").unwrap();
    ...
    let function_name_as_field_1 = function_name_1.to_field().unwrap();
    let function_name_as_field_2 = function_name_2.to_field().unwrap();
    ...
    let r0_dynamic = DynamicRecord::<CurrentNetwork>::from_str(&record_static_str).unwrap();
    ...
    // Authorize the function call.
    let authorization = process
        .authorize::<CurrentAleo, _>(&caller_private_key, program.id(), function_name_2, [r0_value].iter(), rng)
        .unwrap();
    assert_eq!(authorization.len(), 2);
    println!("\nAuthorize\n{:#?}\n\n", authorization.to_vec_deque());
...
fn test_execute_with_translation_case_2() {
    ...
    function consume_liquid:
      input r0 as liquid_container.record;
      call.dynamic {program_id_as_field} {network_as_field} {function_as_field} with r0 (as liquid_container.record) into r1 (as bool.public);
      output r1 as bool.public;
