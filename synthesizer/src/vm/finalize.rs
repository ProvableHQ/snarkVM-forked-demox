...
impl<N: Network, C: ConsensusStorage<N>> VM<N, C> {
    ...
    pub(crate) fn atomic_speculate_inner(
        ...
        atomic_finalize!(self.finalize_store(), FinalizeMode::DryRun, {
            ...
            // Initialize the previous transactions spend.
            let mut previous_transactions_spend = 0;
            ...
            'outer: for transaction in transactions {
                ...
                let compute_spend = match transaction {
                    Transaction::Deploy(_, _, _, ref deployment, _) => {
                        match deployment_cost(self.process(), deployment, consensus_version) {
                            Ok((_, cost_details)) => {
                                match deploy_compute_cost_in_microcredits(cost_details, consensus_version) {
                                    Ok(compute_spend) => compute_spend,
                                    Err(e) => return Err(format!("Failed to compute the deployment cost - {e}")),
                                }
                            }
                            Err(e) => return Err(format!("Failed to compute the deployment cost - {e}")),
                        }
                    }
                    Transaction::Execute(_, _, ref execution, _) => {
                        match execution_cost(self.process(), execution, consensus_version) {
                            Ok((_, cost_details)) => {
                                match execute_compute_cost_in_microcredits(cost_details, consensus_version) {
                                    Ok(compute_spend) => compute_spend,
                                    Err(e) => return Err(format!("Failed to compute the execution cost - {e}")),
                                }
                            }
                            Err(e) => return Err(format!("Failed to compute the execution cost - {e}")),
                        }
                    }
                    Transaction::Fee(..) => 0, // Fee transactions are generated by the VM and do not contribute to spend limits.
                };
                ...
                match outcome {
                    ...
                    Ok(confirmed_transaction) => {
                        ...
                        // Add the compute spend to the previous transactions spend.
                        previous_transactions_spend = previous_transactions_spend.saturating_add(compute_spend);
    ...
    /// - The transaction surpasses the spend limit.
    ///
    ...
    fn should_abort_transaction(
        ...
        previous_transactions_spend: u64,
        ...
        // If the consensus version is >= V15, ensure that the transaction is not exceeding the block spend limit.
        if consensus_version >= ConsensusVersion::V15 {
            ...
            if let Some(block_spend_limit) = block_spend_limit {
                if previous_transactions_spend.saturating_add(compute_spend) > block_spend_limit {
                    return Some(format!(
                        "Transaction '{id}' exceeds the block spend limit with compute_spend: '{compute_spend}'"
                    ));
                }
            }
        }
        ...
        for transaction in transactions.iter() {
            let compute_spend = match transaction {
                Transaction::Deploy(_, _, _, deployment, _) => {
                    let (_, cost_details) = deployment_cost(self.process(), deployment, consensus_version)?;
                    deploy_compute_cost_in_microcredits(cost_details, consensus_version)?
                }
                Transaction::Execute(_, _, execution, _) => {
                    let (_, cost_details) = execution_cost(self.process(), execution, consensus_version)?;
                    execute_compute_cost_in_microcredits(cost_details, consensus_version)?
                }
                // Abort the transaction early if it is a fee transaction.
                Transaction::Fee(..) => {
                    aborted_transactions
                        .push((*transaction, "Fee transactions are not allowed in speculate".to_string()));
                    continue;
                }
            };
