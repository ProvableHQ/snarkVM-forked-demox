                ...
                let compute_spend = match transaction {
                        ...
                        match deployment_cost(self.process(), deployment, consensus_version) {
                            ...
                            Err(e) => {
                                if consensus_version >= ConsensusVersion::V15 {
                                    // NOTE: this can happen when the transaction deploys a program which is already deployed.
                                    aborted.push((
                                        transaction.clone(),
                                        format!("Failed to compute the deployment cost - {e}"),
                                    ));
                                    continue 'outer;
                                } else {
                                    0
                    ...
                    Transaction::Execute(_, _, ref execution, _) => {
                        match execution_cost(self.process(), execution, consensus_version) {
                            ...
                            Err(e) => {
                                if consensus_version >= ConsensusVersion::V15 {
                                    // NOTE: this can happen when the transaction executes a program which is not yet deployed.
                                    aborted.push((
                                        transaction.clone(),
                                        format!("Failed to compute the execution cost - {e}"),
                                    ));
                                    continue 'outer;
                                } else {
                                    0
                        ...
                        Err(_) => {
                            if consensus_version >= ConsensusVersion::V15 {
                                aborted_transactions
                                    .push((*transaction, "Failed to compute the deployment cost".to_string()));
                                continue;
                            } else {
                                0
                            }
                ...
                Transaction::Execute(_, _, execution, _) => {
                    match execution_cost(self.process(), execution, consensus_version) {
                        ...
                        Err(_) => {
                            if consensus_version >= ConsensusVersion::V15 {
                                aborted_transactions
                                    .push((*transaction, "Failed to compute the execution cost".to_string()));
                                continue;
                            } else {
                                0
                            }
