// Copyright (c) 2019-2025 Provable Inc.
// This file is part of the snarkVM library.

// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at:

// http://www.apache.org/licenses/LICENSE-2.0

// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#[cfg(test)]
pub mod tests;

use super::*;

impl<A: Aleo> Record<A, Plaintext<A>> {
    /// Returns the root of the merkleized record data as defined in `DynamicRecord`
    pub fn merkleize(
        &self,
    ) -> Result<Field<A>> {
        let depth = <A::Network as console::Network>::MAX_DATA_ENTRIES.ilog2();

        ensure!(!self.data.is_empty(), "A Record must have at least one entry in order to be merkleized");
        ensure!(
            self.data.len() <= <A::Network as console::Network>::MAX_DATA_ENTRIES,
            "The record exceeds the maximum allowed size ({} > {})",
            self.data.len(),
            <A::Network as console::Network>::MAX_DATA_ENTRIES
        );

        // Initialize the padding value
        let path_hasher = Poseidon2::<A::Network>::setup("DynamicRecordPathHasher")?;
        let padding_hash = Field::<A>::new(Mode::Constant, path_hasher.hash_empty()?);

        let mut level = self.data.iter().map(|(identifier, entry)| {
            let mut hash_input = vec![identifier.to_field()];
            hash_input.extend(entry.to_fields());
            A::hash_psd8(hash_input.as_slice())
        }).collect_vec();         

        for _ in 0..depth {
            //Padding the level to even length
            if level.len() % 2 == 1 {
                level.push(padding_hash.clone());
            }

            // Hashing pairs of nodes
            let next_level = level.chunks_exact(2).map(|left_and_right| {
                A::hash_psd2(left_and_right)
            }).collect_vec();
            level = next_level;
        }

        ensure!(level.len() == 1, "Root level of Merkle tree has {} nodes, expected 1", level.len());
        Ok(level.pop().unwrap())
    }
}

impl<A: Aleo> Record<A, Ciphertext<A>> {
    pub fn merkleize(
        &self,
        _program_id: &ProgramID<A>,
        _record_name: &Identifier<A>,
        _record_view_key: &Field<A>,
    ) -> Field<A> {
        A::halt("Illegal operation: Record::merkleize() cannot be invoked on the `Ciphertext` variant.")
    }
}
