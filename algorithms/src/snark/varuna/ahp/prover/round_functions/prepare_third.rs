...
impl<F: PrimeField, SM: SNARKMode> AHPForR1CS<F, SM> {
    ...
    fn calculate_prep_lineval_sumcheck_witness(
        ...
        anyhow::ensure!(state.circuit_specific_states.len() == fft_precomputations.len(), "[calculate Prep Lineval Sumcheck Witness] Expected {} circuit specific states, but {} were provided.", fft_precomputations.len(), state.circuit_specific_states.len());
        anyhow::ensure!(state.circuit_specific_states.len() == assignments.len(), "[calculate Prep Lineval Sumcheck Witness] Expected {} assignments, but {} were provided.", assignments.len(), state.circuit_specific_states.len());
        anyhow::ensure!(state.circuit_specific_states.len() == matrix_transposes.len(), "[calculate Prep Lineval Sumcheck Witness] Expected {} matrix transposes, but {} were provided.", matrix_transposes.len(), state.circuit_specific_states.len());
