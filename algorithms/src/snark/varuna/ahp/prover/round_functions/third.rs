...
impl<F: PrimeField, SM: SNARKMode> AHPForR1CS<F, SM> {
    ...
    fn calculate_lineval_sumcheck_witness(
        ...
        ensure!(state.circuit_specific_states.len() == third_round_batch_combiners.len(), "[calculate Lineval Sumcheck Witness] Expected {} circuit specific states, but {} were provided.", third_round_batch_combiners.len(), state.circuit_specific_states.len());
        ensure!(state.circuit_specific_states.len() == assignments.len(), "[calculate Lineval Sumcheck Witness] Expected {} assignments, but {} were provided.", assignments.len(), state.circuit_specific_states.len());
        ensure!(state.circuit_specific_states.len() == matrix_transposes.len(), "[calculate Lineval Sumcheck Witness] Expected {} matrix transposes, but {} were provided.", matrix_transposes.len(), state.circuit_specific_states.len());
